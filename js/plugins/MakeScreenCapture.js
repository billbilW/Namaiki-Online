...
(()=> {
    'use strict';
    if (!Utils.isNwjs()) {
        console.warn('MakeScreenCapture: NW.js specific features disabled for web runtime.');
        return;
    }
    ...
    StorageManager.localImgFileDirectoryPath = function() {
        // SCREENSHOT_DIR が定義されていればそれを使い、そうでなければ LocationText を使う
        let filePath = process.env.SCREENSHOT_DIR || param.LocationText;
    
        // %USERPROFILE% を展開する
        filePath = filePath.replace('%USERPROFILE%', process.env.USERPROFILE);
